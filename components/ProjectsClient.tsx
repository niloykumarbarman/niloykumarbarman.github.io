...
const ProjectsClient = ({ projects }: ProjectsClientProps) => {
  ...
  const featuredProjects = useMemo(() => projects.filter(p => p.isFeatured), [projects]);
  const openSourceProjects = useMemo(() => projects.filter(p => p.isOpenSource).length, [projects]);
  ...
  const timelineStats = useMemo(() => {
    ...
    return {
      ...
      featuredCount: filtered.filter(p => p.isFeatured).length,
      openSourceCount: filtered.filter(p => p.isOpenSource).length,
    };
  }, [filteredProjects, selectedSkill]);

  // Pagination logic
  const filteredMainProjects = useMemo(() => filteredProjects.filter(p => !p.isLegacy), [filteredProjects]);
  const filteredLegacyProjects = useMemo(() => filteredProjects.filter(p => p.isLegacy === true), [filteredProjects]);

  const totalPages = Math.ceil(filteredMainProjects.length / PROJECTS_PER_PAGE);
  const paginatedProjects = useMemo(() => {
    const start = (currentPage - 1) * PROJECTS_PER_PAGE;
    return filteredMainProjects.slice(start, start + PROJECTS_PER_PAGE);
  }, [filteredMainProjects, currentPage]);

  // Reset page when filter/search changes
  useEffect(() => {
    setCurrentPage(1);
  }, [filteredProjects, selectedSkill, searchQuery]);
  ...
  return (
    ...
    >
      <BackgroundElements
        floatingDots={[
          { size: "md", color: "secondary", animation: "ping", position: { top: "25%", right: "10%" }, opacity: 60 },
          { size: "sm", color: "blue", animation: "pulse", position: { bottom: "20%", right: "30%" }, opacity: 40 }
        ]}
      />

      <div className="container mx-auto px-4 relative z-10">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6" data-testid="projects-header">
        ...
        {/* Stats - Grid View */}
        {viewMode === "grid" && (
          <motion.div variants={PERFORMANCE_VARIANTS.containerSync} initial="hidden" animate="visible" className="mb-6">
            <StatsCards
              stats={[
                { icon: FaCode, value: totalCount.count, label: "Total Projects", iconColor: "text-[#00BFFF]", iconBgColor: "bg-[#00BFFF]/20", valueGradient: "from-[#00BFFF] to-[#0080FF]", ref: totalCount.ref },
                { icon: FaClock, value: activeCount.count, label: "Active", iconColor: "text-emerald-400", iconBgColor: "bg-emerald-500/20", valueGradient: "from-emerald-400 to-cyan-500", ref: activeCount.ref },
                { icon: FaRocket, value: featuredCount.count, label: "Featured", iconColor: "text-purple-400", iconBgColor: "bg-purple-500/20", valueGradient: "from-purple-400 to-pink-500", ref: featuredCount.ref },
                { icon: FaGithub, value: openSourceCount.count, label: "Open Source", iconColor: "text-blue-400", iconBgColor: "bg-blue-500/20", valueGradient: "from-blue-400 to-secondary-default", ref: openSourceCount.ref }
              ]}
              showDividers={true}
            />
          </motion.div>
        ...
        {viewMode === "timeline" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <StatsCards
              stats={[
                { icon: FaCode, value: timelineStats.total, label: "Total Projects", iconColor: "text-[#00BFFF]", iconBgColor: "bg-[#00BFFF]/20", valueGradient: "from-[#00BFFF] to-[#0080FF]" },
                { icon: FaClock, value: timelineStats.activeCount, label: "Active", iconColor: "text-emerald-400", iconBgColor: "bg-emerald-500/20", valueGradient: "from-emerald-400 to-cyan-500" },
                { icon: FaRocket, value: timelineStats.featuredCount, label: "Featured", iconColor: "text-purple-400", iconBgColor: "bg-purple-500/20", valueGradient: "from-purple-400 to-pink-500" },
                { icon: FaGithub, value: timelineStats.openSourceCount, label: "Open Source", iconColor: "text-blue-400", iconBgColor: "bg-blue-500/20", valueGradient: "from-blue-400 to-secondary-default" }
              ]}
              showDividers={true}
            />
          </motion.div>
        ...
        {/* Timeline View */}
        {viewMode === "timeline" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} data-testid="timeline-view">
            <ProjectTimeline
              projects={sortedProjects}
        ...
        {viewMode === "grid" && (
          <>
            ...
            {selectedSkill && (
              ...
              >
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <span className="text-white font-medium">Filtered by skill:</span>
            ...
            <ErrorBoundary section="projects">
              <div data-testid="projects-grid" className="mt-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {isLoading ? (
            ...
            {!isLoading && totalPages > 1 && (
              ...
              >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border border-white/10 bg-white/5 hover:bg-white/10 text-white/70 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200"
                  ...
                  return (
                    <button
                      ...
                      className={`w-9 h-9 rounded-lg text-sm font-semibold transition-all duration-200 ${
                        isActive
                          ? "bg-gradient-to-r from-secondary-default to-blue-500 text-white shadow-md shadow-secondary-default/30"
            ...
            {!isLoading && totalPages > 1 && (
              <p className="text-center text-xs text-white/40 mt-3">
                Showing {((currentPage - 1) * PROJECTS_PER_PAGE) + 1}–{Math.min(currentPage * PROJECTS_PER_PAGE, filteredMainProjects.length)} of {filteredMainProjects.length} projects
              </p>
            )}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <h2 className="text-lg font-semibold text-white/80">Other Projects</h2>
                  ...
                  <div className="flex items-center gap-2 text-white/60">
                    <span className="text-sm hidden sm:inline">{showEarlierProjects ? 'Hide' : 'Show'}</span>
                    {showEarlierProjects ? <FaChevronUp className="text-sm" /> : <FaChevronDown className="text-sm" />}
                  </div>
                </button>
