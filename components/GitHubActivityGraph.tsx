...
interface GitHubActivityGraphProps {
  ...
  /**
   * Optional override:
   * If you want to show a different GitHub account without touching lib/github,
   * pass username here.
   */
  username?: string;

  /**
   * Optional override for profile link.
   * If not provided, it will use GITHUB_PROFILE_URL from lib/github.
   */
  profileUrl?: string;
}
...
export default function GitHubActivityGraph({
  ...
  const GH_USERNAME = username || GITHUB_USERNAME;
  const GH_PROFILE_URL = profileUrl || GITHUB_PROFILE_URL;
  ...
  const [graphQLContributions, setGraphQLContributions] =
  ...
  // Avoid hydration mismatch (dates + animations)
  useEffect(() => {
    setHydrated(true);
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const { stats: githubStats, contributionMap } =
          await fetchGitHubStatsWithContributions(GH_USERNAME);
  ...
  }, [GH_USERNAME, onStatsLoaded]);

  // GraphQL data takes priority (1 year), fallback to Events API (90 days)
  ...
  const gridData = useMemo(() => (hydrated ? generateGridData() : []), [
    generateGridData,
    hydrated,
  ]);
  ...
  // Loading skeleton (also shown before hydration)
          ...
          <div className="min-w-[900px] max-w-[1100px] mx-auto">
            ...
            <div className="flex gap-[3px] mb-1 ml-8">
              {gridData.map((_, weekIndex) => {
                ...
                return (
                  <div
                    key={weekIndex}
                    className="w-3.5 text-[10px] text-white/50"
                  >
                    {monthLabel}
                  </div>
                );
            ...
            <div className="flex gap-1">
              ...
              <div
                ...
                role="img"
                aria-label="GitHub contribution graph showing activity over the past year"
              >
                  ...
                  <div key={weekIndex} className="flex flex-col gap-[3px]">
                    {week.map((date, dayIndex) => {
                      ...
                      return (
                        ...
                        >
                          {/* Tooltip */}
                          {hoveredCell?.x === weekIndex &&
                            hoveredCell?.y === dayIndex && (
                              <motion.div
            ...
            <div className="flex items-center justify-end gap-2 mt-3 text-[10px] text-white/60">
              <span>Less</span>
              <div
                className="w-3.5 h-3.5 rounded-sm bg-white/5 border border-white/10"
                aria-hidden="true"
              />
              <div
                className="w-3.5 h-3.5 rounded-sm bg-emerald-500/30 border border-emerald-400/50"
                aria-hidden="true"
              />
              <div
                className="w-3.5 h-3.5 rounded-sm bg-emerald-500/60 border border-emerald-400/70"
                aria-hidden="true"
              />
              <div
                className="w-3.5 h-3.5 rounded-sm bg-emerald-500/80 border border-emerald-400/90"
                aria-hidden="true"
              />
              <div
                className="w-3.5 h-3.5 rounded-sm bg-emerald-500 border border-emerald-400"
                aria-hidden="true"
              />
              <span>More</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:flex sm:flex-wrap sm:items-center sm:justify-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-[#00BFFF]/20 rounded-lg">
                  <span className="text-[#00BFFF] text-lg" aria-hidden="true">
                    🎯
                  </span>
                </div>
                <div>
              ...
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-gradient-to-br from-orange-500/20 to-red-500/20 rounded-lg">
                  <span className="text-orange-400 text-lg" aria-hidden="true">
                    🔥
                  </span>
                </div>
                <div>
              ...
              <div
                className="hidden lg:block w-px h-8 bg-white/10"
                aria-hidden="true"
              />
              ...
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-purple-500/20 rounded-lg">
                  <span className="text-purple-400 text-lg" aria-hidden="true">
                    🏆
                  </span>
                </div>
                <div>
